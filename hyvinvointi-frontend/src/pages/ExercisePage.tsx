...
const ExercisePage: React.FC = () => {
  ...
  const handleSubmit = async (data: any) => {
    try {
      if (editingEntry) {
        await exerciseService.update(editingEntry.id, data);
      } else {
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">Liikunta</h1>
          {!showForm && (
            <button
        ...
        {loading ? (
          ...
          <ExerciseList entries={entries} onDelete={handleDelete} onEdit={handleEdit} />
        )}
      </div>
    </div>
  );
